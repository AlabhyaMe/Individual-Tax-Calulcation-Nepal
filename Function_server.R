...
Salary_Function <- function(SalaryM, Status, ret_tab){
  ...
  while(i<=12){
    if(CummulativeIncome[i]<=TB[1]){                     #Slab 1 Condition
      ...
      if (abs(CummulativeIncome[i]-TB[4]) < Salary[i]){
        Tax4[i] <- abs(CummulativeIncome[i]-TB[4])*0.36
        Tax3[i] <- (Salary[i]-abs(CummulativeIncome[i]-TB[4]))*0.3
      }
      else{
        Tax4[i]<- Salary[i]*0.30
      }
  ...
  Tax_Table <- data.frame(Month,Salary,CummulativeIncome,Tax1,Tax2,Tax3,Tax4)
  Tax_Table$TotalMonthlyTax <- rowSums(Tax_Table[,c(4:7)])
  ...
  Stacked_Table <- Tax_Table %>%
                      gather(key = "TaxBracket", value = "Amount", 4:7)
